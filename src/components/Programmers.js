...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    const featuredProgrammer = pioneers.find(dev => dev.id === currentId);
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: currentId !== null ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          pioneers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setCurrentId(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
